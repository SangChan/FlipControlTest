...
class FlipControl1: UIView {
    ...
    fileprivate func initializer() {
        ...
        let frontView = FlipView1(number: timer, frame: CGRect(x: 0, y: 0, width: self.frame.size.width * 0.9, height: self.frame.size.height*0.9))
    ...
    func fliped() {
        self.timer -= 1
        let newbackView = FlipView1(number: timer, frame: CGRect(x: 0, y: 0, width: self.frame.size.width * 0.9, height: self.frame.size.height*0.9))
        newbackView.center = CGPoint(x: self.frame.size.width/2, y: self.frame.size.height/2)
        
        let frontView = self.subviews.first as! FlipView1
